...
object FrameQualityGate {
    ...
    fun isCorruptBlackFrame(bitmap: Bitmap): Boolean {
        return isCorruptBlackPixels(bitmap.width, bitmap.height) { x, y -> bitmap.getPixel(x, y) }
    }
    ...
    fun isCorruptBlackPixels(width: Int, height: Int, getPixel: (x: Int, y: Int) -> Int): Boolean {
        ...
        val averageLuminance = totalLuminance / totalCount
        return fractionBlack >= FRACTION_BLACK_THRESHOLD && averageLuminance <= AVERAGE_LUMINANCE_THRESHOLD
    }
}
