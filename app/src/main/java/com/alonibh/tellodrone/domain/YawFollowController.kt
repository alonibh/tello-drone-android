 ...
 * It never predicts target motion: stale, discontinuous, or crossing input brakes to zero.
 */
class ProductionYawController(
    ...
    private val maximumYawRcStep: Int = MAXIMUM_YAW_RC_STEP,
    ...
    init {
        ...
        require(maximumYawRcStep in 1..absoluteYawRcCap)
    ...
    fun command(errors: TrackingErrors?, commandTimestampNanos: Long): YawControlOutcome {
        ...
        if (lastFrame != null && lastSource != null) {
            ...
            if (sameMeasurement) {
                val requested = requestedYaw(measurement.filteredYawError)
                return outcome(
                    measurement,
                    commandTimestampNanos,
        ...
        val crossedCenter = previousFiltered != null && previousYaw != 0 && (
            previousFiltered * measurement.filteredYawError < 0f ||
        ...
        val safetyYaw = when {
            requested == 0 -> 0
            else -> requested.coerceIn(previousYaw - maximumYawRcStep, previousYaw + maximumYawRcStep)
        }
        lastYawRc = safetyYaw
        return outcome(
            measurement,
    ...
    private fun observe(measurement: Measurement) {
        ...
        lastFilteredYawError = measurement.filteredYawError
    }

    private fun requestedYaw(error: Float): Int = if (abs(error) <= horizontalDeadband) 0 else {
        (error * proportionalGain).roundToInt().coerceIn(-absoluteYawRcCap, absoluteYawRcCap)
    }
    ...
    private fun outcome(
        ...
        return YawControlOutcome(
            ...
            filteredYawError = measurement.filteredYawError,
            previousYawRc = previousYawRc,
            requestedYawRc = requestedYawRc,
            safetyFilteredYawRc = safetyFilteredYawRc,
            suppressionReason = suppressionReason,
            validForMillis = (remainingNanos / NANOS_PER_MILLISECOND).coerceAtLeast(0L),
        )
    }
