...
/** One-shot UI gate for the real-aircraft confirmation dialog. */
internal class TakeoffConfirmationGate {
    private var awaitingConfirmation = false

    fun request(state: DroneSessionState): Boolean {
        awaitingConfirmation = state.controllerMode == ControllerMode.Real && state.isTakeoffEligible()
    ...
    fun dismissIfIneligible(state: DroneSessionState): Boolean {
        if (awaitingConfirmation && (state.controllerMode != ControllerMode.Real || !state.isTakeoffEligible())) {
            awaitingConfirmation = false
        }
        return awaitingConfirmation
    }
    ...
    fun confirm(state: DroneSessionState, onConfirmed: () -> Unit): Boolean {
        ...
        awaitingConfirmation = false
        if (state.controllerMode != ControllerMode.Real || !state.isTakeoffEligible()) return false
        onConfirmed()
        return true
    }
}
